...
public class ServerState extends ActionBarActivity {
	View serverPage, seatPage;	//보여줄 페이지 주문현황 좌석현황 
	...
	static int Value1;				// 그때 그때 클릭한 position의 orderMenuID 저장변수. 
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //전체화면 만들기
    	Window swin = getWindow();
    	swin.requestFeature(Window.FEATURE_NO_TITLE);
    	swin.addFlags(WindowManager.LayoutParams.FLAG_FULLSCREEN);
    	setContentView(R.layout.serverstate);
    	
        
        serverPage = findViewById(R.id.serverpage);
        seatPage = findViewById(R.id.seatpage);
        
        findViewById(R.id.serverbtn).setOnClickListener(mClickListener);
        ...
        //서빙목록 파싱필요 버튼 클릭이벤트 선언.
        Button p2Button = (Button)this.findViewById(R.id.parse1);	//새로고침할 버튼 변수 정의 "새로고침"
        p2Button.setOnClickListener(new Button.OnClickListener(){
        	public void onClick(View v){ //버튼 누르면 현재 서빙 상황 새로고침
        		new ReadJSONFeed2().execute("http://117.16.231.212:8010/getCookedList");	//읽어올 url
        	}
        });
        ...
        timer1.schedule(myTask1, 10000, 1000); 	//10초 기다리고 1초마다 새로고침        
        
        //좌석현황 파싱필요 버튼 클릭이벤트 선언.
        Button pButton1 = (Button)this.findViewById(R.id.parse);	
        //새로고침할 버튼 변수 정의 "좌석현황"
        pButton1.setOnClickListener(new Button.OnClickListener(){
        	public void onClick(View v){ 
        		//버튼 누르면 현재 주문 상황 새로고침
        		new ReadJSONFeed3().execute("http://117.16.231.212:8010/tableState");	//읽어올 url
        	}
        });
        ...
        timer.schedule(myTask, 10000, 1000); 	//10초 기다리고 1초마다 새로고침
    ...
    Button.OnClickListener mClickListener = new Button.OnClickListener(){
    	public void onClick(View v){
    		serverPage.setVisibility(View.INVISIBLE);
    ...
    AdapterView.OnItemClickListener mItemClickListener = new AdapterView.OnItemClickListener(){
    	public void onItemClick(AdapterView<?> parent, View view, int position, long id){
    		...
    		if(position != ListView.INVALID_POSITION)
    		{
    			...
    			Value1 = ID[position-1];
    			//푸쉬사용위해 선언부분.
    	    	//서빙 완료.
    	    	sendJsonToServer_list2 list2;								//리스트 변수 선언.
    	    	list2 = new sendJsonToServer_list2();						//리스트 변수 객체 생성.
    	    	list2.execute("http://117.16.231.212:8010/finishServe");		// 푸쉬 URL 설정.
    ...
    public static String JsonPost_list(){
		...
		try{
		list = jsonStringer.object().key("orderMenuID").value(Value1).endObject().toString();
	...
	private class sendJsonToServer_list2 extends AsyncTask<String, String, String> {
	    ...
	    protected String doInBackground(String... urls) {
			...
			try {
				...
				conn.setConnectTimeout(15*1000);	//쉼표 하나 제거. 2에서 15
				conn.setReadTimeout(10*1000);	//1에서 10
